                ...
                match message.type:
                    ...
                    case MessageType.SALE_START:
                        if message.data.get("success"):
                            ...
                            self.logger.info(f"Sale started successfully: {self.current_item}")
                        with self.response_lock:
                    ...
                    case MessageType.STOCK_UPDATE:
                        ...
                        if (self.current_item and 
                            message.data.get("item_id") == self.current_item.get("item_id")):
                            self.current_item.update({
                                "quantity": message.data["quantity"],
                                "remaining_time": message.data.get("remaining_time", 60.0)
                            })
                            self.logger.debug(f"Updated item status: {self.current_item}")
